...
function delCallbackFromMap(key: string, callBack: Function, _map: Map<string, Function[]>) {
  ...
  calbacks = calbacks.filter(function (v: any): boolean {
    return v !== callBack
  })
  _map.set(key, calbacks)
}
...
export class TrpcCon implements IrpcCon {
  ...
  onWsMsg(ev: MessageEvent | OnSocketMessageCallbackResult): void {
    ...
    switch (rpcMsg.Cmd) {
      ...
      case 14:
        ...
        break
      case 15:
        // server push ws msg, not nats msg
        this.onServerWsMsg(rpcMsg)
        break
      case 44:
        break
    }
  ...
  onServerWsMsg(rpc: yrpcmsg.Ymsg) {
    // 事件主题固定为"msg_pkg_t"
    const nats_subject = rpc.Optstr
    const nats_body = rpc.Body
    const nats_reply = rpc.Optbin

    // 将事件广播出去，上层应用使用rpcCon.on监听
    StrPubSub.publish(nats_subject, nats_body, nats_subject, nats_reply, rpc)
  }
  ...
  onNatsMsg(rpc: yrpcmsg.Ymsg) {
    ...
    if (onceSubsFn.length > 0) {
      ...
      for (let j = 0; j < onceSubsFn.length; ++j) {
        onceSubsFn[j].apply(this, [nats_body, nats_subject, nats_reply, rpc])
      }
    }
    ...
    if (subsFn.length === 0) {
      ...
      for (let i = 0; i < subsFn.length; ++i) {
        subsFn[i].apply(this, [nats_body, nats_subject, nats_reply, rpc])
      }
    }
  }
